...
BEGIN TRY
    ...
    IF NOT EXISTS (
        ...
        BEGIN 
            ...
            SET @boolCatchFlag = 1;
            SET @strErrorMessage = 'Attempt to add comment on column ' + @strColumnName + ' of ' + @strTableName + 
                 ';however, either ' + @strColumnName + ' or ' + @strTableName + ' does not exist, check spelling, try again?' ;
...
IF @boolCatchFlag = 1
    BEGIN 
    	INSERT INTO dbo.DB_EXCEPTION_TANK(UserName
                                        , ErrorState
                                        , ErrorSeverity
                                        , ErrorProcedure
                                        , ErrorMessage
